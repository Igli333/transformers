...
def sparsemixer(scores, top_k, jitter_eps, training):
    ...
    if training:
        selected_experts_top2 = (
                masked_gates_top2 - torch.empty_like(masked_gates_top2,
                                                     memory_format=torch.legacy_contiguous_format).exponential_().log()
        ).max(dim=-1)[1].unsqueeze(-1)  # gumbel sampling, more robust than than the multinomial method
    else:
...
class MultiPolicyRouter(nn.Module):
    ...
    def forward(self, logits, training=True, input_ids=None):
        ...
        Returns:
           ...
           selected_experts: [tokens, k] (indices)
        """

        if self.policy == "top_1":
            # classical Switch Transformer routing
...
class PhiMoESparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        gates, selected_experts = self.router(router_logits, training=self.training)
        routing_weights = gates
...
class PhiMoEForCausalLM(PhiMoEPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values and self.config.rope_scaling and input_ids.shape[
            1] >= self.config.original_max_position_embeddings + 1:
            past_length = past_key_values.seen_tokens if isinstance(past_key_values, Cache) else \
