...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        # Handle HF's annoying (None, None) past_key_value tuples
...
class TopKBalancedNoisyGate(nn.Module):
    def __init__(
        ...
        # add_noise (used only for noisy_topk / original routing)
        self.add_noise = add_noise
        self.noise_epsilon = noise_epsilon
        self.warned = False

        # NEW: routing policy name
        # valid values: "topk_noisy", "topk", "top1", "switch", "soft", "hash"
    ...
    def _route_topk_noisy(self, logits, logits_gate, x, logits_noise, noise_control):
        """
        Original behavior: noisy Top-K with +1 extra for load estimation.
        """
        ...
        return top_k_indices, top_k_scores, top_logits

    def _route_topk(self, logits_gate):
        """
        Deterministic Top-K (no noise).
        """
        top_logits, top_indices = logits_gate.topk(
            min(self.num_selects, self.num_experts),
            dim=1,
        )
        ...
        top_k_scores = top_k_scores.to(logits_gate.dtype)
        top_k_indices = top_indices
        return top_k_indices, top_k_scores

    def _route_top1(self, logits_gate):
        """
        Top-1 gating. If use_softmax=True, weight is just 1.
        """
        top_k_logits, top_k_indices = logits_gate.topk(1, dim=1)  # [B, 1]
        if self.use_softmax:
            top_k_scores = torch.ones_like(top_k_logits, dtype=logits_gate.dtype)
        else:
            top_k_scores = top_k_logits.to(logits_gate.dtype)
        return top_k_indices, top_k_scores

    def _route_soft(self, logits_gate):
        """
        Soft gating: softmax over all experts, then select top-K probabilities.
        Not fully dense (to keep num_selects fixed) but closer to soft routing.
        """
        probs = torch.softmax(logits_gate.to(torch.float32), dim=1).to(logits_gate.dtype)
        top_k_scores, top_k_indices = probs.topk(
            min(self.num_selects, self.num_experts),
            dim=1,
        )
        return top_k_indices, top_k_scores

    def _route_hash(self, logits_gate):
       """
       Hash-based static routing (safe version):
       Produces int64 expert indices in [0, num_experts).
       """
       batch_size = logits_gate.size(0)

       # deterministic hash in [0, num_experts)
       hash_vals = (logits_gate > 0).to(torch.int32).sum(dim=1)  # [B] int
       experts = (hash_vals % self.num_experts).to(torch.int64)  # [B] int64

       # repeat across num_selects to keep shape [B, K]
       top_k_indices = experts.unsqueeze(1).repeat(1, self.num_selects)  # int64
       top_k_scores = torch.full(
        (batch_size, self.num_selects),
        1.0 / self.num_selects,
        device=logits_gate.device,
        dtype=logits_gate.dtype,
       )
       return top_k_indices, top_k_scores
    ...
    def forward(self, x):
        ...
        policy = getattr(self, "routing_policy", "topk_noisy")

        # Select routing behavior
        if policy == "topk_noisy":
            top_k_indices, top_k_scores, top_logits = self._route_topk_noisy(
                logits,
                logits_gate,
                x,
                logits_noise,
            ...
            top_k_indices, top_k_scores = self._route_topk(logits_gate)
            top_logits = None
        elif policy == "top1" or policy == "switch":
            top_k_indices, top_k_scores = self._route_top1(logits_gate)
            top_logits = None
        elif policy == "soft":
            top_k_indices, top_k_scores = self._route_soft(logits_gate)
            top_logits = None
        elif policy == "hash":
            top_k_indices, top_k_scores = self._route_hash(logits_gate)
            top_logits = None
        else:
            raise ValueError(f"Unknown routing_policy: {policy}")
        
        # Build full scores tensor over all experts (same as original)
        ...
        # ================= SAFETY CHECKS =================
        # Prevent CUDA device-side asserts from invalid indices
        if top_k_indices.dtype != torch.int64:
         top_k_indices = top_k_indices.to(torch.int64)

        assert int(top_k_indices.min()) >= 0, "Negative expert index detected"
        assert int(top_k_indices.max()) < self.num_experts, (
         f"Expert index out of range: max={int(top_k_indices.max())}, "
         f"num_experts={self.num_experts}"
        )
        # =================================================

        scores_filtered = zeros.scatter(dim=1, index=top_k_indices, src=top_k_scores)
        importance = scores_filtered.sum(0)  # [num_experts]


        # Load computation & balance loss (mostly original logic)
        if self.training:
            # Only use fancy noise-based load for the original noisy top-k
            if (
                self.add_noise
                and self.num_selects != self.num_experts
                and policy == "topk_noisy"
                and top_logits is not None
                and logits_noise is not None
                and noise_control is not None
            ):
        ...
        **kwargs,
        ...
        if self.multiply_gate_scores:
            if self.mlp_norm is None:
                ...
                cat_expert_outputs = torch.mul(
                    cat_expert_outputs,
                    sorted_topK_scores.reshape(-1, 1),
                )
                self.mlp_norm(cat_expert_outputs)
    ...
    def _create_gate(self, **kwargs):
        ...
        if self.gate_type == "TopKBalancedNoisyGate":
            self.gate = TopKBalancedNoisyGate(
                ...
                routing_policy=kwargs.get("routing_policy", "topk_noisy"),
            )
        else:
            raise NotImplementedError
...
class LlamaMoEDecoderLayer(nn.Module):
    def __init__(self, config: LlamaMoEConfig, layer_index):
        ...
        gating_config = {
            ...
            # NEW: routing policy from config
            "routing_policy": getattr(config, "routing_policy", "topk_noisy"),
        }
        calculator_config = {
...
class LlamaMoEModel(LlamaMoEPreTrainedModel):
    ...
    def forward(
        ...
        # robust handling of possibly-None caches for generation
        if (
    ...
    def update_config(self):
        ...
        # persist routing policy back to config
        self.config.routing_policy = vars(self.layers[0].mlp.gate).get(
            "routing_policy",
            getattr(self.config, "routing_policy", "topk_noisy"),
        )
    ...
    # NEW: clean runtime API for routing policy
...
class LlamaMoEForCausalLM(LlamaMoEPreTrainedModel):
    ...
    # NEW: pass-through for routing policy
    def set_moe_routing_policy(self, routing_policy: str):
