...
from .configuration_phimoe import PhimoeAISConfig
...
class PhimoeAISRotaryEmbedding(nn.Module):
    def __init__(
        self,
        config: Optional[PhimoeAISConfig] = None,
    ):
        super().__init__()
...
class PhimoeAISAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """

    def __init__(self, config: PhimoeAISConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class PhimoeAISFlashAttention2(PhimoeAISAttention):
    """
    Phimoe flash attention module. This module inherits from `PhimoeAISAttention` as the weights of the module stays
...
class PhimoeAISSdpaAttention(PhimoeAISAttention):
    """
    Phimoe attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `PhimoeAISAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from PhimoeAISAttention.forward
    @deprecate_kwarg("past_key_value", new_name="past_key_values", version="4.58")
    def forward(
        self,
        hidden_states: torch.Tensor,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "PhimoeAISModel is using PhimoeAISSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
PHIMOE_ATTENTION_CLASSES = {
    "eager": PhimoeAISAttention,
    "flash_attention_2": PhimoeAISFlashAttention2,
    "sdpa": PhimoeAISSdpaAttention,
}


# Copied from transformers.models.mixtral.modeling_mixtral.MixtralBlockSparseTop2MLP with Mixtral->Phimoe
class PhimoeAISBlockSparseTop2MLP(nn.Module):
    def __init__(self, config: PhimoeAISConfig):
        super().__init__()
...
class PhimoeAISSparseMoeBlock(nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList([PhimoeAISBlockSparseTop2MLP(config) for _ in range(self.num_experts)])
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # ---- routing policy dispatch (Option A: keep Top-2 internal layout) ----
        policy = getattr(self, "routing_policy", None)
        if policy is None:
            # fallback to config if attribute missing
            policy = getattr(self, "config", None)
            if policy is not None:
                policy = getattr(self.config, "routing_policy", "top_k")
            else:
                policy = "top_k"

        if policy == "top_k":
            # Use sparsemixer with no jitter by default (deterministic Top-2 selection)
            routing_weights, selected_experts = sparsemixer(
                router_logits,
                jitter_eps=0,
                training=self.training,
            )
        elif policy == "noisy_top_k":
            # Let sparsemixer handle the jitter internally
            routing_weights, selected_experts = sparsemixer(
                router_logits,
            ...
            # Select top-1 and duplicate the index to match Top-2 layout; second weight set to 0
            top1 = torch.argmax(router_logits, dim=-1, keepdim=True)
            # Create selected_experts [N,2] with duplicate indices
            selected_experts = torch.cat((top1, top1), dim=-1)
            weights1 = torch.softmax(router_logits, dim=-1).gather(dim=-1, index=top1)
            zero = torch.zeros_like(weights1)
            routing_weights = torch.cat((weights1, zero), dim=-1)
        elif policy == "soft":
            # Dense routing: compute softmax, then pick top-2 experts per token but keep their soft weights
            probs = torch.softmax(router_logits, dim=-1)
            topk_vals, topk_idx = torch.topk(probs, 2, dim=-1)
            # Use the gathered probabilities as routing multipliers (not renormalized)
            routing_weights = torch.zeros((probs.size(0), 2), device=probs.device, dtype=probs.dtype)
            routing_weights[:, 0] = topk_vals[:, 0]
            routing_weights[:, 1] = topk_vals[:, 1]
            selected_experts = topk_idx
        elif policy == "adaptive":
            # Example adaptive: learned or fixed temperature (use config if available)
            tau = getattr(self, "router_temperature", None) or getattr(getattr(self, "config", None), "router_temperature", 0.7)
            probs = torch.softmax(router_logits / tau, dim=-1)
            topk_vals, topk_idx = torch.topk(probs, 2, dim=-1)
            routing_weights = torch.zeros((probs.size(0), 2), device=probs.device, dtype=probs.dtype)
            routing_weights[:, 0] = topk_vals[:, 0]
            routing_weights[:, 1] = topk_vals[:, 1]
            selected_experts = topk_idx
        elif policy == "load_balanced_top_k":
            # Use sparsemixer and let top-k selection happen; load balancing is computed outside using router logits
            routing_weights, selected_experts = sparsemixer(
                router_logits,
                jitter_eps=self.router_jitter_noise,
                training=self.training,
            )
            # store raw logits for downstream aux loss calculation (the model collects router_logits already)
        elif policy == "hash":
            # Deterministic static hash: pick two experts via two simple hashes
            N = router_logits.size(0)
            ex1 = (torch.arange(N, device=router_logits.device) * 9973) % self.num_experts
            ex2 = (ex1 + 1) % self.num_experts
            selected_experts = torch.stack((ex1, ex2), dim=-1)
            routing_weights = torch.zeros((N, 2), device=router_logits.device, dtype=router_logits.dtype)
            routing_weights[:, 0] = 1.0
            routing_weights[:, 1] = 0.0
        elif policy == "token_based":
            # Switch-like token-based top-1 routing: top-1 duplicated for Top-2 layout
            top1 = torch.argmax(router_logits, dim=-1, keepdim=True)
            selected_experts = torch.cat((top1, top1), dim=-1)
            weights1 = torch.softmax(router_logits, dim=-1).gather(dim=-1, index=top1)
            zero = torch.zeros_like(weights1)
            ...
            # default fallback to sparsemixer top-2
            routing_weights, selected_experts = sparsemixer(
                router_logits,
                jitter_eps=self.router_jitter_noise,
                training=self.training,
            )
        # ensure routing_weights and selected_experts have expected shapes ([N,2] and [N,2])
...
class PhimoeAISDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: PhimoeAISConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = PHIMOE_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx)

        self.block_sparse_moe = PhimoeAISSparseMoeBlock(config)
...
@auto_docstring
class PhimoeAISPreTrainedModel(PreTrainedModel):
    config: PhimoeAISConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["PhimoeAISDecoderLayer"]
...
@auto_docstring
class PhimoeAISModel(PhimoeAISPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`PhimoeAISDecoderLayer`]
    Args:
        config: PhimoeAISConfig
    """
    ...
    def __init__(self, config: PhimoeAISConfig):
        ...
        self.layers = nn.ModuleList(
            [PhimoeAISDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = nn.LayerNorm(config.hidden_size, eps=config.rms_norm_eps, elementwise_affine=True)
        self.rotary_emb = PhimoeAISRotaryEmbedding(config=config)
...
class PhimoeAISForCausalLM(PhimoeAISPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = PhimoeAISModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        Example:
        ```python
        >>> from transformers import AutoTokenizer, PhimoeAISForCausalLM
        >>> model = PhimoeAISForCausalLM.from_pretrained("microsoft/Phi-3.5-MoE-instruct")
...
class PhimoeForSequenceClassification(GenericForSequenceClassification, PhimoeAISPreTrainedModel): ...


__all__ = [
    "PhimoeAISPreTrainedModel",
    "PhimoeAISModel",
    "PhimoeAISForCausalLM",
